...
const getPermissions = state => state.manager.permissions.permissions;
const getOriginalPermissions = state =>
  state.manager.permissions.originalPermissions;
...
function getTooltipForGroup(group) {
  if (isAdminGroup(group)) {
    return t`Administrators always have the highest level of access to everything in Foundry.`;
...
export const getGroups = createSelector(
  state => state.manager.permissions.groups,
  groups => {
...
export const getSaveError = state => state.manager.permissions.saveError;
    ...
    return {
      ...
      crumbs:
        database.schemaNames().length > 1
          ? [
            [t`Databases`, "/manager/permissions/databases"],
            [
              database.name,
              "/manager/permissions/databases/" + database.id + "/schemas",
            ],
            [schemaName],
          ]
          : [[t`Databases`, "/manager/permissions/databases"], [database.name]],
      groups,
      permissions: {
        fields: {
...
export const getSchemasPermissionsGrid = createSelector(
  ...
  (
    ...
    return {
      type: "schema",
      icon: "folder",
      crumbs: [[t`Databases`, "/manager/permissions/databases"], [database.name]],
      groups,
      permissions: {
        tables: {
          header: t`Data Access`,
          ...
          postAction(groupId, { databaseId, schemaName }, value) {
            if (value === "controlled") {
              return push(
                `/manager/permissions/databases/${databaseId}/schemas/${encodeURIComponent(
                  schemaName,
                )}/tables`,
              );
            }
      ...
      entities: schemaNames.map(schemaName => ({
        ...
        link: {
          name: t`View tables`,
          url: `/manager/permissions/databases/${databaseId}/schemas/${encodeURIComponent(
            schemaName,
          )}/tables`,
        },
      })),
          ...
          postAction(groupId, { databaseId }, value) {
            if (value === "controlled") {
              ...
              if (
                ...
                return push(
                  `/manager/permissions/databases/${databaseId}/tables`,
                );
              } else if (schemas.length === 1) {
                return push(
                  `/manager/permissions/databases/${databaseId}/schemas/${
                  schemas[0]
                  }/tables`,
                );
              } else {
        ...
        return {
          ...
          link:
            schemas.length === 0 || (schemas.length === 1 && schemas[0] === "")
              ? {
                name: t`View tables`,
                url: `/manager/permissions/databases/${database.id}/tables`,
              }
              : schemas.length === 1
                ? {
                  name: t`View tables`,
                  url: `/manager/permissions/databases/${database.id}/schemas/${
                    schemas[0]
                    }/tables`,
                }
                : {
...
export const getPropagatePermissions = state =>
  state.manager.permissions.propagatePermissions;
    ...
    if (parent) {
      while (parent) {
        if (crumbs.length > 0) {
          crumbs.unshift([
            parent.name,
            `/manager/permissions/collections/${parent.id}`,
          ]);
        } else {
      ...
      crumbs.unshift([t`Collections`, "/manager/permissions/collections"]);
    }
      ...
      entities: collections.map(collection => {
        return {
          ...
          link: collection.children &&
            ...
            url: `/manager/permissions/collections/${collection.id}`,
          },
        };
      }),
    };
