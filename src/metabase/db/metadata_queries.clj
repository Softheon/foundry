...
(def ^:private ^Integer absolute-max-distinct-values-limit
  ...
  Admins can however manually mark any Field as `list`, which is effectively ordering Foundry to keep FieldValues for
  the Field regardless of its cardinality.

  Of course, if a User does something crazy, like mark a million-arity Field as List, we don't want Foundry to
  ...
  * Not being too high, which would result in Foundry running out of memory dealing with too many values"
  (int 5000))
