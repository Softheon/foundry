...
(defn update-settings-last-updated!
  ...
  (let [current-timestamp-as-string-honeysql (hx/cast (if (= (mdb/db-type) :mysql) :char :text)
        ...
        current-timestamp-as-string (or current-timestamp-as-string-honeysql-mssql
                                        current-timestamp-as-string-honeysql)]
    ;; attempt to UPDATE the existing row. If no row exists, `update-where!` will return false...
    (or (db/update-where! 'Setting {:key settings-last-updated-key} :value current-timestamp-as-string)
        ...
        (try
          ...
          (db/simple-insert! 'Setting :key settings-last-updated-key, :value current-timestamp-as-string)
