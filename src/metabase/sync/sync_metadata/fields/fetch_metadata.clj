(ns metabase.sync.sync-metadata.fields.fetch-metadata
  "Logic for constructing a map of metadata from the Foundry application database that matches the form of DB metadata
...
(s/defn fields->our-metadata :- #{common/TableMetadataFieldWithID}
  "Given a sequence of Foundry Fields, format them and return them in a hierachy so the format matches the one
...
(s/defn ^:private table->fields :- [i/FieldInstance]
  "Fetch active Fields from the Foundry application database for a given `table`."
