(ns metabase.sync.sync-metadata.fields.sync-instances
  "Logic for syncing the instances of `Field` in the Foundry application DB with the set of Fields in the DB metadata.
...
(s/defn ^:private matching-inactive-fields :- (s/maybe [i/FieldInstance])
  "Return inactive Foundry Fields that match any of the Fields described by `new-field-metadatas`, if any such Fields
  exist."
