(ns metabase.sync.sync-metadata.fields
  ...
  * Fetch Metadata - logic is in `metabase.sync.sync-metadata.fields.fetch-metadata`. Construct a map of metadata from
    the Foundry application database that matches the form of DB metadata about Fields in a Table. This metadata is
  ...
  * Sync Field instances -- logic is in `metabase.sync.sync-metadata.fields.sync-instances`. Make sure the `Field`
    instances in the Foundry application database match up with those in the DB metadata, creating new Fields as
  ...
  *  `our-metadata` is a set of maps of Field metadata reconstructed from the Foundry application database.

  *  `metabase-field` is a single map of Field metadata reconstructed from the Foundry application database; there is
     a 1:1 correspondance between this metadata and a row in the `Field` table. Unlike `field-metadata`, these entries
     always have an `:id` associated with them (because they are present in the Foundry application DB).
...
(s/defn ^:private sync-and-update! :- su/IntGreaterThanOrEqualToZero
  "Sync Field instances (i.e., rows in the Field table in the Foundry application DB) for a Table, and update metadata
