...
(def ^:dynamic *db-connection*
  "Bind this to override the default DB connection used by `metabase.toucan.db` functions. Provided for situations where you'd
...
(defn set-default-db-connection!
  ...
  the various `metabase.toucan.db` functions.
...
(defn resolve-model
  "Resolve a model *if* it's quoted. This also unwraps entities when they're inside vectores.
     ...
     (resolve-model 'Database)        -> #'my-project.models.database/Database"
  [model]
  {:post [(:metabase.toucan.models/model %)]}
  (cond
    (:metabase.toucan.models/model model) model
...
(defn- mssql-boolean-converter
  [value]
  (if (instance? Boolean value)
    (if (true? value) 1 0))
  value)

(defn- sqlserver-args-converter
  [value]
  (-> value
      (mssql-boolean-converter)))

(defn- convert-sql-args
  [args]
  (if (= :sqlserver (quoting-style))
    (doall
     (map #(sqlserver-args-converter %1) args))
    args))


(defn- mssql-page-converter
  [honeysql-form] 
  (let [skip (:offset honeysql-form)
        fetch (:limit honeysql-form)]
    (-> honeysql-form
        (dissoc :offset)
        (dissoc :limit)
...
(defn- honeymssql-page
  [honeysql-form]
  (if (and (contains? honeysql-form :limit)
           (contains? honeysql-form :offset))
    (mssql-page-converter honeysql-form)
    honeysql-form))


(defn- honeymssql-limit
  [honeysql-form]
  (if (contains? honeysql-form :limit)
    (let [limit-value  (:limit honeysql-form)
          select-values (:select honeysql-form)]
      (-> honeysql-form
          (dissoc :limit)
          (h/modifiers (keyword (str "TOP " limit-value)))))
    honeysql-form))
...
(defn- honeysql->honeymsql
  [honeysql-form]
  (if (= :sqlserver (quoting-style))
    (honeymsql honeysql-form)
    honeysql-form))

(defn- db-sql-format
  [[sql & args :as sql+args]]
  (let [update-args  (convert-sql-args args)
        update-sql (convert-sql sql)]
    (into [update-sql] update-args)))
...
(defn honeysql->sql
  ...
  (let [[sql & args :as sql+args]
        (binding [hformat/*subquery?* false]
         (db-sql-format  (hsql/format (honeysql->honeymsql honeysql-form)
...
(defn-  limit [m l]
  (if (nil? l)
    m
    (assoc m :top (if (coll? l) (first l) l))))
