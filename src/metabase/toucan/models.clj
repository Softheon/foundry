...
(defn- invoke-model
  ...
  ([model]
   ((resolve 'metabase.toucan.db/select) model))
  ([model id]
   (when id
     (invoke-model model (primary-key model) id)))
  ([model k v & more]
   (apply (resolve 'metabase.toucan.db/select-one) model k v more)))
...
(defmacro defmodel
  ...
  the model or instances fetched from the DB.
     ...
     (db/select User, ...)  ; use with `metabase.toucan.db` functions. All results are instances of `UserInstance`
    ...
    `(do
       ...
       (extend ~instance
         ~@(mapcat identity (merge-with (fn [this that] `(merge ~this ~that))
                              `{metabase.toucan.models/IModel         metabase.toucan.models/IModelDefaults
                                metabase.toucan.models/ICreateFromMap {:map-> (fn [~'_ & args#] (apply ~map->instance args#))}
