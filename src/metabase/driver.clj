...
(defn- load-driver-namespace-if-needed
  "Load the expected namespace for a `driver` if it has not already been registed. This only works for core Foundry
...
the driver, but do not connect to databases; these can be be supplied, for example, by a Foundry plugin manifest
...
Foundry drivers in v1.0 and above) are automatiaclly given an implentation of this method that performs the
