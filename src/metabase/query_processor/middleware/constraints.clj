(ns metabase.query-processor.middleware.constraints
  "Middleware that adds default constraints to limit the maximum number of rows returned to queries
that specify the `:add-default-userland-constraints?` `:middleware` options.")
...
(defn- merge-default-constraint [constraints]
...
(defn- add-default-userland-constraints*
  ...
  (cond-> query
    add-default-userland-constraints? (update :constraints (comp ensure-valid-constraints merge-default-constraint))))
