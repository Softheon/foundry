(ns metabase.plugins.init-steps
  "Logic for performing the `init-steps` listed in a Foundry plugin's manifest. For driver plugins that specify that we
  should `lazy-load`, these steps are lazily performed the first time non-trivial driver methods (such as connecting
  to a Database) are called; for all other Foundry plugins these are perfomed during launch.
...
(defn do-init-steps!
  "Perform the initialization steps for a Foundry plugin as specified under `init:` in its plugin
