...
(defmethod dependency-satisfied? :default [_ {{plugin-name :name} :info} dep]
  (log/error
   (u/format-color 'red
       (trs "Plugin {0} declares a dependency that Foundry does not understand: {1}" plugin-name dep))
   (trs "Refer to the plugin manifest reference for a complete list of valid plugin dependencies:")
   "https://github.com/metabase/metabase/wiki/Foundry-Plugin-Manifest-Reference")
  false)
...
(defn- warn-about-required-dependencies [plugin-name message]
  (log-once plugin-name
    (str (u/format-color 'red (trs "Foundry cannot initialize plugin {0} due to required dependencies." plugin-name))
         " "
         message)))
