     ...
     (try
       ...
       (u/prog1 (files/get-path filename)
         ...
         (assert (Files/isWritable <>)
           (str (trs "Foundry does not have permissions to write to plugins directory {0}" filename))))
       ...
       (catch Throwable e
         (log/warn
          e
          (trs "Foundry cannot use the plugins directory {0}" filename)
          "\n"
          (trs "Please make sure the directory exists and that Foundry has permission to write to it.")
          (trs "You can change the directory Foundry uses for modules by setting the environment variable MB_PLUGINS_DIR.")
         ...
         (u/prog1 (files/get-path (System/getProperty "java.io.tmpdir"))
           (assert (Files/isWritable <>)
             (str (trs "Foundry cannot write to temporary directory. Please set MB_PLUGINS_DIR to a writable directory and restart Foundry.")))))))))
...
(defn- plugins-dir
  "Get a `Path` to the Foundry plugins directory, creating it if needed. If it cannot be created for one reason or
  ...
  (for [^Path path (files/files-seq (plugins-dir))
        :when      (and (files/regular-file? path)
                        ...
                        (or (not (str/ends-with? (.getFileName path) "spark-deps.jar"))
                            ...
                            ;; the Foundry JAR itself as well as plugins no longer are; Java will throw an Exception
                            ...
                            (log/warn
                             (u/format-color 'red
                                 (trs "spark-deps.jar is no longer needed by Foundry 1.0+. You can delete it from the plugins directory.")))))]
    path))
...
(defn- init-plugins! [paths]
  ...
  ;; driver `ojdbc8.jar`) always get initialized (i.e., added to the classpath) first; that way, Foundry drivers that
  ;; depend on them (such as Oracle) can be initialized the first time we see them.
  ;;
  ;; In Clojure world at least `false` < `true` so we can use `sort-by` to get non-Foundry-plugin JARs in front
...
(defn load-plugins!
  "Load Foundry plugins. The are JARs shipped as part of Foundry itself, under the `resources/modules` directory (the
  source for these JARs is under the `modules` directory); and others manually added by users to the Foundry plugins
  directory, which defaults to `./plugins`.

  When loading plugins, Foundry performs the following steps:
./b
  *  Foundry creates the plugins directory if it does not already exist.
  *  Any plugins that are shipped as part of Foundry itself are extracted from the Foundry uberjar (or `resources`
     directory when running with `lein`) into the plugins directory.
  *  Each JAR in the plugins directory is added to the classpath.
  *  For JARs that include a Foundry plugin manifest (a `metabase-plugin.yaml` file), "
  []
